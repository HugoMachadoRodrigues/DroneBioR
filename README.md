...
`lidR` and its dependency `rlas` were **removed from CRAN on 9 June 2026**
(`rlas` for uncorrected ASAN issues, `lidR` as a dependent), so
...
This affects only the point-cloud path — CSF ground refinement, the rebuilt
DTM and CHM, and point-cloud metrics. Everything else (reading products,
radiometric scaling, vegetation indices, windowed field extraction, model
fitting and prediction) works without `lidR`, and the functions that do need it
say so with the install command above rather than failing deep in the call.
